...
def main():
    ...
    # with open('/datax/scratch/calebp/k_scores/SORTED_FILTERED_LOW_OUTLIERS_OFF_25_ON_5.pkl', 'rb') as f:
    #     low_off_outliers = pickle.load(f)
    # all_outliers = pd.DataFrame(low_off_outliers)
    ...
    if 'actually drifting' not in all_outliers.columns: 
        all_outliers['actually drifting'] = pd.Series(dtype='object')
    ...
    filenames = []
    obs_infos = []
    for num in tqdm(range(0,len(all_outliers))):
        try:
            filename, obs_info,actually_drifting = single_plot_wrapper(num,all_outliers)
            print(obs_info)
            all_outliers.at[num, 'info'] = str(obs_info)
            all_outliers.at[num, 'actually drifting'] = actually_drifting
            all_outliers.to_csv("/datax/scratch/calebp/k_scores/updated_SORTED_FILTERED_LOW_OUTLIERS_OFF_25_ON_5.csv")
        except Exception:
