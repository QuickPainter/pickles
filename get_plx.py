...
def main():
    ...
    for num in tqdm(range(len(unique_names))):
        result_table = simbad.query_object(unique_names[num])
        try:
            parallaxes.append(result_table["PLX_VALUE"][0])
        except Exception as e:
            print(e)
            parallaxes.append(np.nan)

    names_and_parallaxes= [names,parallaxes]
    with open("/datax/scratch/calebp/k_scores/all_file_parallaxes3.pkl", 'wb') as handle:
        pickle.dump(names_and_parallaxes, handle, protocol=pickle.HIGHEST_PROTOCOL)
