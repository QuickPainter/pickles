...
def main():
    ...
    all_observation_snrs = []

    
    for num in tqdm(range(len(low_off_outliers))):
        k_regions = low_off_outliers
        all_files = np.array(k_regions["All Files"])[num]
        batch_info = np.array(k_regions["Batch Info"])[num]
        drift2 = np.array(k_regions["drift2"])[num]
        ...
        hf1 = h5py.File(all_files[0], 'r')
        hf2 = h5py.File(all_files[1], 'r')
        hf3 = h5py.File(all_files[2], 'r')
        hf4 = h5py.File(all_files[3], 'r')
        hf5 = h5py.File(all_files[4], 'r')
        hf6 = h5py.File(all_files[5], 'r')
        
        fch1, foff, nchans, ra, decl, target = get_file_properties(hf1)
        block_size = int(np.array(k_regions["Block Size"])[num])
        i = np.array(k_regions["Index"])[num]
        freq = np.array(k_regions["Freq"])[num]
        i = ((freq-fch1)/foff)/block_size
        ...
        all_observation_snrs.append(total_signal_snr)


    with open("/datax/scratch/calebp/k_scores/all_outlier_snrs_run_final.pkl", 'wb') as handle:
        pickle.dump(all_observation_snrs, handle, protocol=pickle.HIGHEST_PROTOCOL)
